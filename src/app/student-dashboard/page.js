...
export default function StudentDashboard() {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 w-full animate-pulse">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-5 sm:mb-8">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className="h-[90px] sm:h-[100px] bg-slate-200 rounded-2xl"
            ></div>
          ))}
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-sm sm:text-base text-slate-500 font-medium mb-8">
          Please Login to view your personalized analytics and history.
        </p>
        <button
          className="px-6 sm:px-8 py-3 sm:py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md shadow-blue-600/20 transition-all active:scale-95"
          onClick={() => router.push("/")}
        >
          Go to Home
        </button>
  ...
  myHistory.forEach((h) => {
    ...
    if (!weaknessData[h.subject])
      weaknessData[h.subject] = { mistakes: 0, attempts: 0 };
  ...
  if (totalTests > 0) {
    if (overallPercentage >= 85) {
      ...
      rankTitle = "Consistent Pro";
      rankColor = "text-indigo-700";
      rankBg = "bg-indigo-100";
      rankIcon = "ti-trending-up";
    } else if (overallPercentage >= 50) {
      rankTitle = "Developing Learner";
      rankColor = "text-emerald-700";
      rankBg = "bg-emerald-100";
      rankIcon = "ti-barbell";
    } else {
      rankTitle = "Needs Focus";
      rankColor = "text-rose-700";
      rankBg = "bg-rose-100";
      rankIcon = "ti-alert-circle";
    }
  }
  ...
  const recentTrend = reversedHistory
  ...
  if (totalTests > 0) {
    if (overallAccuracy >= 80)
      smartInsights.push({
        ...
        desc: "Your hit rate is top-notch. Keep avoiding blind guesses.",
        color: "emerald",
      });
    else if (overallAccuracy < 50)
      smartInsights.push({
        icon: "ti-minus",
        title: "Watch the Negatives",
        desc: "Losing marks to wrong answers. Skip if completely unsure.",
        color: "rose",
      });

    if (totalTests >= 3 && overallPercentage > 75)
      smartInsights.push({
        icon: "ti-flame",
        title: "On Fire!  ",
        desc: "Consistently scoring high. You are ready for tougher exams.",
        color: "amber",
      });
      ...
      smartInsights.push({
        icon: "ti-trending-up",
        title: "Keep Grinding",
        desc: "Review past mistakes in the ledger below to improve.",
        color: "indigo",
      });
  }
    ...
    <div className="bg-slate-900 rounded-3xl p-5 sm:p-8 shadow-2xl relative overflow-hidden flex flex-col md:flex-row justify-between gap-6 sm:gap-8 border border-slate-800">
      ...
      <div className="relative z-10 flex items-center gap-4 sm:gap-6">
        ...
        <div className="flex-1">
          <h1 className="text-xl sm:text-3xl font-black text-white mb-1.5 sm:mb-2 tracking-tight leading-tight">
          ...
          <div className="flex flex-col gap-1.5 w-full max-w-[200px] sm:max-w-xs mt-2">
            <div className="flex justify-between items-center">
              <span className="text-[9px] sm:text-[11px] font-extrabold text-blue-400 uppercase tracking-widest bg-blue-500/10 px-2 py-0.5 rounded">
                Level {currentLevel}
              </span>
              <span className="text-[9px] sm:text-[11px] font-bold text-slate-400">
                {testsToNextLevel} test(s) to Lvl {currentLevel + 1}
              </span>
            </div>
      ...
      {totalTests > 0 && (
        <div className="relative z-10 flex items-center gap-3 sm:gap-4 shrink-0 bg-white/5 backdrop-blur-md border border-white/10 p-3 sm:p-4 sm:pr-6 rounded-2xl md:self-end">
          <div
            className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center text-xl sm:text-3xl ${rankBg} ${rankColor} shadow-inner`}
          >
            <i className={`ti ${rankIcon}`}></i>
          </div>
          <div>
            <div className="text-[9px] sm:text-[11px] font-extrabold text-slate-400 uppercase tracking-widest leading-none mb-1 sm:mb-1.5">
              Current Rank
            </div>
            <div
              className={`text-[14px] sm:text-[18px] font-black ${rankColor.replace("text-", "text-").replace("-700", "-400")} leading-none tracking-tight`}
            >
              {rankTitle}
            </div>
          </div>
  ...
  const bentoStatsCard = (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
      <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-[0_2px_10px_rgb(0,0,0,0.02)] flex flex-col items-center text-center group hover:border-blue-200 transition-colors">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center text-xl sm:text-2xl mb-2 sm:mb-3 border border-blue-100 group-hover:scale-110 transition-transform">
          <i className="ti ti-file-certificate"></i>
        </div>
        <div className="text-xl sm:text-2xl font-black text-slate-800 mb-1 leading-none">
          {totalTests}
        </div>
        <div className="text-[9px] sm:text-[10px] font-extrabold text-slate-500 uppercase tracking-widest leading-tight">
          Exams Taken
        </div>
      </div>
      <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-[0_2px_10px_rgb(0,0,0,0.02)] flex flex-col items-center text-center group hover:border-emerald-200 transition-colors">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-xl sm:text-2xl mb-2 sm:mb-3 border border-emerald-100 group-hover:scale-110 transition-transform">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-[0_2px_10px_rgb(0,0,0,0.02)] flex flex-col items-center text-center group hover:border-amber-200 transition-colors">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center text-xl sm:text-2xl mb-2 sm:mb-3 border border-amber-100 group-hover:scale-110 transition-transform">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-[0_2px_10px_rgb(0,0,0,0.02)] flex flex-col items-center text-center group hover:border-rose-200 transition-colors">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center text-xl sm:text-2xl mb-2 sm:mb-3 border border-rose-100 group-hover:scale-110 transition-transform">
    ...
    <div className="bg-white p-4 sm:p-7 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-[0_4px_20px_rgb(0,0,0,0.03)] h-full flex flex-col">
      <div className="flex items-center justify-between mb-6 sm:mb-8 shrink-0">
        <h3 className="text-[14px] sm:text-[18px] font-black text-slate-800 flex items-center gap-2 sm:gap-2.5 m-0 tracking-tight">
          <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100">
            <i className="ti ti-chart-bar text-lg sm:text-xl"></i>
          </div>
          Performance Trend
        </h3>
      </div>
      ...
      <div className="w-full overflow-x-auto custom-scrollbar pb-2 flex-1 flex flex-col justify-end">
        <div className="flex items-end gap-2 sm:gap-4 h-[160px] sm:h-[200px] min-w-[380px] sm:min-w-[500px] border-b-2 border-slate-100 px-1 sm:px-2 relative">
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-40">
            <div className="border-t border-dashed border-slate-300 w-full h-0 relative">
              <span className="absolute -top-2.5 -left-1 text-[8px] sm:text-[9px] font-bold text-slate-400">
                100%
              </span>
            </div>
            <div className="border-t border-dashed border-slate-300 w-full h-0 relative">
          ...
          {recentTrend.map((h, i) => {
            ...
            let barColor = isExcellent
              ? "border-emerald-500"
              : isAverage
                ? "border-amber-400"
                : "border-rose-400";
            let barBg = isExcellent
              ? "bg-gradient-to-t from-emerald-100 to-emerald-400"
              : isAverage
                ? "bg-gradient-to-t from-amber-100 to-amber-400"
                : "bg-gradient-to-t from-rose-100 to-rose-400";
            let textColor = isExcellent
              ? "text-emerald-700"
              : isAverage
                ? "text-amber-700"
                : "text-rose-700";
            ...
            return (
              ...
              >
                <div
                  className={`text-[10px] sm:text-[12px] font-black ${textColor} mb-1 sm:mb-2 transition-all duration-700 delay-[${i * 50}ms] ${animateGraph ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
                >
                  {pct}%
                </div>
                <div
                  className={`w-full max-w-[30px] sm:max-w-[44px] rounded-t-lg sm:rounded-t-xl border-t border-x ${barColor} ${barBg} cursor-pointer relative overflow-hidden group-hover:-translate-y-1 transition-transform shadow-sm`}
                  style={{
                    height: animateGraph ? `${Math.max(pct, 5)}%` : "0%",
    ...
    <div className="bg-slate-50 p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-inner h-full flex flex-col">
      <div className="flex items-center justify-between mb-4 sm:mb-5 shrink-0">
        <h3 className="text-[13px] sm:text-[14px] font-extrabold text-slate-800 uppercase tracking-widest flex items-center gap-2 m-0">
          <i className="ti ti-brain text-amber-500 text-lg sm:text-xl"></i> AI
          Analysis
        </h3>
        <select
          value={analysisMode}
      ...
      <div className="flex-1 flex flex-col gap-3 sm:gap-4 overflow-y-auto custom-scrollbar pr-1">
        {analysisMode === "insights" &&
          smartInsights.map((insight, idx) => (
            <div
              key={idx}
              className={`bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border flex items-start gap-3 shadow-sm border-${insight.color}-200`}
            >
              <div
                className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-${insight.color}-50 text-${insight.color}-600 flex items-center justify-center text-lg sm:text-xl shrink-0`}
              >
                <i className={`ti ${insight.icon}`}></i>
              </div>
              <div>
                <h4
                  className={`text-[13px] sm:text-[14px] font-black text-${insight.color}-800 mb-0.5 sm:mb-1 leading-tight`}
                >
                  {insight.title}
                </h4>
                <p className="text-[11px] sm:text-[12px] font-medium text-slate-600 leading-snug">
                  {insight.desc}
                </p>
              </div>
            </div>
        ...
        {analysisMode === "weakness" && (
          <div className="flex flex-col gap-3">
            {Object.keys(weaknessData).length === 0 ? (
              <div className="text-[12px] font-semibold text-slate-500 text-center py-4">
              ...
              Object.keys(weaknessData).map((sub, idx) => {
                ...
                const mistakePct =
                  d.attempts > 0
                    ? Math.round((d.mistakes / d.attempts) * 100)
                    : 0;
                let barColor =
                  mistakePct > 40
                    ? "bg-rose-500"
                ...
                return (
                  <div
                    key={idx}
                    className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm"
                  >
                    <div className="flex justify-between items-center mb-1.5 sm:mb-2">
                      <div className="text-[12px] sm:text-[13px] font-bold text-slate-700 truncate pr-2">
                        <i className="ti ti-book text-slate-400 mr-1"></i> {sub}
                      </div>
                      <div className="text-[10px] sm:text-[11px] font-extrabold text-slate-500">
                        {mistakePct}% Errors
                      </div>
                    </div>
