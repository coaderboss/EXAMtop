...
export default function CreateTest() {
  ...
  const handleRestoreDraft = () => {
    ...
    if (draft.toggles) {
      setAllowChange(
        draft.toggles.change !== undefined ? draft.toggles.change : true,
      );
      setShowPalette(
        draft.toggles.palette !== undefined ? draft.toggles.palette : true,
      );
      ...
      setAntiCheat(
        draft.toggles.anticheat !== undefined ? draft.toggles.anticheat : true,
      );
  ...
  const toggleCorrectMSQ = (qIdx, optIdx, isChecked) => {
    ...
    if (isChecked) newList[qIdx].correct.push(optIdx);
      ...
      newList[qIdx].correct = newList[qIdx].correct.filter((x) => x !== optIdx);
    setQList(newList);
  };
      ...
      questions: [
        {
          ...
          figureType: "svg",
          figureData:
            "<svg viewBox='0 0 200 100' width='100%' height='150' xmlns='http://www.w3.org/2000/svg'><path d='M20 50 L50 50 L55 40 L65 60 L75 40 L85 60 L90 50 L120 50' stroke='black' stroke-width='2' fill='none'/><text x='10' y='55' font-family='Arial'>A</text><text x='130' y='55' font-family='Arial'>B</text><text x='65' y='30' font-family='Arial'>5 Ω</text></svg>",
          options: ["2 Ω", "5 Ω", "10 Ω", "15 Ω"],
          correct: [1],
          correctInt: null,
        },
        {
          ...
          figureType: "tikz",
          figureData:
            "\\begin{tikzpicture}\\draw[->] (0,0) -- (4,0) node[right] {$t$}; \\draw[->] (0,0) -- (0,3) node[above] {$v$}; \\draw[thick, blue] (0,0) parabola (3,2.5);\\end{tikzpicture}",
          options: [],
          correct: [],
          correctInt: 8,
        },
        {
          ...
          figureType: "none",
          figureData: "",
          options: ["Friction", "Gravity", "Viscous Drag", "Electrostatic"],
          correct: [0, 2],
          correctInt: null,
        },
        {
          ...
          options: ["Aspirin", "Paracetamol", "Benzene", "Phenol"],
          correct: [0],
          correctInt: null,
        },
      ],
    };
    const blob = new Blob([JSON.stringify(universalTemplate, null, 2)], {
  ...
  const validateAndImportJSON = (rawText) => {
    ...
    try {
      ...
      setJsonModal({
        text: rawText,
        errors: [
          `🚨 Syntax Error: ${e.message}. Please check for missing commas, unclosed brackets, or extra quotes.`,
        ],
      });
      return;
    }
    ...
    let qArray = Array.isArray(parsedData)
      ? parsedData
    ...
    if (!qArray || !Array.isArray(qArray)) {
      setJsonModal({
        text: rawText,
        errors: [
          "🚨 Structure Error: JSON must be an Array of questions OR an object containing a 'questions' array.",
        ],
      });
      return;
    }
    ...
    qArray.forEach((q, idx) => {
      ...
      if (!q.text || q.text.trim() === "")
        errors.push(
          `❌ Question ${qNum}: Missing 'text' (Question body cannot be empty).`,
        );
      if (!q.type || !["mcq", "msq", "integer", "subjective"].includes(q.type))
        errors.push(
          `❌ Question ${qNum}: Invalid type. Must be 'mcq', 'msq', 'integer', or 'subjective'.`,
        );

      if (q.type === "mcq" || q.type === "msq") {
        if (!q.options || !Array.isArray(q.options) || q.options.length < 2)
          errors.push(
            `❌ Question ${qNum} (${q.type}): Must have an 'options' array with at least 2 choices.`,
          );
        if (!q.correct || !Array.isArray(q.correct) || q.correct.length === 0)
          errors.push(
            `❌ Question ${qNum} (${q.type}): Missing 'correct' array (e.g., [0] for A).`,
          );
      }
      ...
      if (
        ...
        errors.push(
          `❌ Question ${qNum} (integer): Missing 'correctInt' value.`,
        );
      }
    ...
    if (errors.length > 0) {
      ...
      setSysAlert({
        ...
        msg: `${mappedData.length} questions mapped perfectly from JSON.`,
        type: "success",
      });
    }
  };
  ...
  const handleImageUpload = (e, qIndex) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setSysAlert({
        ...
        msg: "Please keep images under 2MB for seamless offline execution.",
        type: "warning",
      });
      return;
    }
  ...
  const saveTest = () => {
    ...
    if (!isOffline && !currentUser) {
      setSysAlert({
        ...
        msg: "Login Required! You need to log in to save this test to the cloud.",
        type: "error",
      });
      return;
    }
    if (!title.trim()) {
      setSysAlert({
        title: "Missing Field",
        msg: "Please enter a test title before saving.",
        type: "warning",
      });
      return;
    }
    if (!qList.length) {
      setSysAlert({
        title: "Empty Test",
        msg: "Please add at least one question to the test.",
        type: "warning",
      });
      return;
    }
    ...
    for (let i = 0; i < qList.length; i++) {
      ...
      if (q.type === "mcq" || q.type === "msq") {
        if (!q.correct || q.correct.length === 0) {
          setSysAlert({
            title: "Action Required",
            msg: `Please mark the correct option for Question ${i + 1}.`,
            type: "warning",
          });
          return;
        }
      ...
      if (q.type === "integer") {
        if (
          ...
          setSysAlert({
            title: "Action Required",
            msg: `Please enter the correct integer answer for Question ${i + 1}.`,
            type: "warning",
          });
          return;
        }
  ...
  if (!isOffline && userRole !== "examiner" && userRole !== "admin") {
    return (
      <div style={{ textAlign: "center", padding: "4rem" }}>
        <h3>Access Denied</h3>
        <p>Only Examiners can create tests.</p>
      </div>
    );
  }

  const parsedSectionsArray = sections
    .split(",")
    .map((s) => s.trim())
    .filter((s) => s);
    ...
    <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 animate-[fadeIn_0.3s_ease] pb-[30vh]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div>
          ...
          <div className="mt-3 flex gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[11px] font-extrabold uppercase tracking-widest flex items-center gap-1.5 transition-colors border ${isOffline ? "bg-amber-100 text-amber-700 border-amber-200" : "bg-emerald-100 text-emerald-700 border-emerald-200"}`}
            >
              <i
                className={`ti ${isOffline ? "ti-wifi-off" : "ti-cloud"} text-sm`}
              ></i>
        ...
        <div className="flex flex-wrap sm:flex-nowrap gap-2 sm:gap-3 w-full sm:w-auto mt-2 sm:mt-0">
          <button
            className="flex-1 sm:flex-none px-4 py-2.5 bg-white border border-slate-200 text-blue-600 font-bold text-[13px] rounded-xl shadow-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 active:scale-95"
            onClick={downloadTemplate}
          >
            <i className="ti ti-download text-lg"></i>{" "}
            <span className="hidden sm:inline">JSON</span> Template
          </button>

          {/* NAYA: Paste JSON Button */}
          <button
            className="flex-1 sm:flex-none px-4 py-2.5 bg-white border border-slate-200 text-indigo-600 font-bold text-[13px] rounded-xl shadow-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 active:scale-95"
            onClick={() => {
              setPasteText("");
              setPasteModal(true);
            }}
          >
            <i className="ti ti-clipboard text-lg"></i> Paste{" "}
          ...
          <label className="flex-1 sm:flex-none px-4 py-2.5 bg-white border border-slate-200 text-emerald-600 font-bold text-[13px] rounded-xl shadow-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 cursor-pointer active:scale-95">
            <i className="ti ti-upload text-lg"></i> Import{" "}
            <span className="hidden sm:inline">JSON</span>
            <input
              type="file"
              accept=".json"
            ...
            return (
              ...
              >
                ...
                {isProcessingSave ? (
                  ...
                  <>
                    <i className="ti ti-ban text-lg"></i> Zero Tokens Left
                  </>
                ) : (
                  <>
      ...
      {pendingDraft && (
        <div className="bg-amber-50 border border-amber-200 p-4 sm:p-5 rounded-2xl mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm animate-[slideDown_0.3s_ease]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-[15px] font-black text-amber-900 leading-none mb-1.5">
                Unsaved Draft Recovered
              </h4>
              <p className="text-[12px] font-semibold text-amber-700/80 leading-none">
                Contains {pendingDraft.qList ? pendingDraft.qList.length : 0}{" "}
                questions. Continue from where you left off?
              </p>
            </div>
          </div>
          <div className="flex gap-2 w-full sm:w-auto shrink-0">
            <button
              ...
              className="flex-1 sm:flex-none px-5 py-2.5 bg-white border border-amber-200 text-amber-700 text-[13px] font-bold rounded-xl hover:bg-amber-100 transition-colors active:scale-95"
            >
              Discard
            </button>
            <button
              onClick={handleRestoreDraft}
              className="flex-1 sm:flex-none px-5 py-2.5 bg-amber-500 text-white text-[13px] font-bold rounded-xl shadow-md shadow-amber-500/20 hover:bg-amber-600 transition-colors active:scale-95 flex items-center justify-center gap-1.5"
            >
              <i className="ti ti-refresh text-base"></i> Restore
            </button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 mb-8">
        ...
        <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col">
          <h3 className="text-[13px] font-extrabold text-slate-800 uppercase tracking-widest mb-5 flex items-center gap-2">
            <i className="ti ti-settings text-blue-500 text-lg"></i> Basic
            Settings
          </h3>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100 transition-all"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:border-blue-400 transition-all"
              />
            </div>
            <div>
              <label className="text-[12px] font-bold text-slate-600 mb-1.5 block">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-blue-400 transition-all"
                />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNegMarking(Math.abs(Number(e.target.value)))
                  }
                  placeholder="0.25"
                  className="w-full pl-10 pr-4 py-3 bg-rose-50/50 border border-slate-200 rounded-xl text-sm font-bold text-rose-700 outline-none focus:border-rose-400 transition-all"
                />
              </div>
            </div>
            <div>
          ...
          <div className="mt-auto">
            <label className="text-[12px] font-bold text-slate-600 mb-1.5 block">
              Test Access / Visibility
            </label>
            <div className="relative">
              <select
                value={access}
                onChange={(e) => setAccess(e.target.value)}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-blue-400 transition-all appearance-none cursor-pointer"
              >
        ...
        <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col">
          <h3 className="text-[13px] font-extrabold text-slate-800 uppercase tracking-widest mb-5 flex items-center gap-2">
            <i className="ti ti-shield-lock text-emerald-500 text-lg"></i>{" "}
            Security & Experience
          </h3>
            ...
            {[
              {
                ...
                desc: "Students can edit marked answers",
                state: allowChange,
                set: setAllowChange,
              },
              {
                label: "Show Question Palette",
                desc: "Display navigation map during exam",
                state: showPalette,
                set: setShowPalette,
              },
              {
                label: "Free Navigation",
                desc: "Allow jumping between questions",
                state: allowNav,
                set: setAllowNav,
              },
              {
                label: "Shuffle Questions",
                desc: "Randomize order for each student",
                state: randomOrder,
                set: setRandomOrder,
              },
              {
                label: "Shuffle Options",
                desc: "Randomize A/B/C/D order",
                state: shuffleOpts,
                set: setShuffleOpts,
              },
            ].map((item, idx) => (
              ...
              >
                ...
                <div
                  className={`w-10 h-5 sm:w-11 sm:h-6 rounded-full cursor-pointer relative transition-colors duration-300 shrink-0 border border-black/5 ${item.state ? "bg-blue-600" : "bg-slate-300"}`}
                  onClick={() => item.set(!item.state)}
                >
                  <div
                    className={`absolute top-[2px] w-4 h-4 sm:w-5 sm:h-5 bg-white rounded-full shadow-sm transition-transform duration-300 ${item.state ? "translate-x-5" : "translate-x-0.5"}`}
                  ></div>
                </div>
              </div>
            ))}
            ...
            {[
              {
                ...
                desc: "Block tab switching and copy-paste",
                state: antiCheat,
                set: setAntiCheat,
              },
              {
                label: "Enforce Full-Screen",
                desc: "Warn if full-screen is exited",
                state: fullScreenMode,
                set: setFullScreenMode,
              },
            ].map((item, idx) => (
              <div
                key={idx}
              ...
              >
                <div>
                  <div className="font-bold text-[13px] text-rose-700">
                    <i className="ti ti-shield-half-filled"></i> {item.label}
                  </div>
                  <div className="text-[11px] font-medium text-rose-600/70">
                    {item.desc}
                  </div>
                </div>
                <div
                  className={`w-10 h-5 sm:w-11 sm:h-6 rounded-full cursor-pointer relative transition-colors duration-300 shrink-0 border border-black/5 ${item.state ? "bg-rose-500" : "bg-slate-300"}`}
                  onClick={() => item.set(!item.state)}
                >
                  <div
                    className={`absolute top-[2px] w-4 h-4 sm:w-5 sm:h-5 bg-white rounded-full shadow-sm transition-transform duration-300 ${item.state ? "translate-x-5" : "translate-x-0.5"}`}
      ...
      {parsedSectionsArray.length > 0 && (
        <div className="bg-white p-5 sm:p-6 rounded-2xl border border-indigo-100 shadow-sm mb-8 relative overflow-hidden">
          ...
          <h3 className="text-[13px] font-extrabold text-indigo-900 uppercase tracking-widest mb-2 flex items-center gap-2 relative z-10">
            <i className="ti ti-list-check text-indigo-500 text-lg"></i>{" "}
            Optional Questions
          </h3>
          <p className="text-[12px] font-semibold text-slate-500 mb-5 leading-relaxed relative z-10 max-w-3xl">
            Define the maximum number of questions a student needs to attempt in
            each section (e.g. Attempt any 5 out of 10). Leave empty if all
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 relative z-10">
            {parsedSectionsArray.map((sec) => (
              <div
                key={sec}
                className="flex items-center justify-between p-3.5 bg-indigo-50/50 border border-indigo-100 rounded-xl"
              >
                <span className="text-[13px] font-bold text-indigo-900 truncate pr-2">
                  <i className="ti ti-folder text-indigo-400 mr-1"></i> {sec}
                </span>
      ...
      <div className="flex flex-col gap-6">
        {qList.map((q, i) => (
          <div
            key={q.id}
            className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all hover:border-blue-300 hover:shadow-md group"
          >
            ...
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-100 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3">
              <div className="flex items-center gap-2.5 w-full sm:w-auto overflow-x-auto custom-scrollbar pb-1 sm:pb-0">
                <div className="w-8 h-8 bg-gradient-to-br from-blue-600 to-indigo-600 text-white rounded-lg flex items-center justify-center font-black text-sm shrink-0 shadow-sm">
                  {i + 1}
                </div>
                ...
                <select
                  ...
                  className="bg-white border border-slate-200 text-slate-700 font-bold text-[12px] rounded-lg px-3 py-1.5 outline-none focus:border-blue-400 shadow-sm cursor-pointer shrink-0"
                >
                ...
                {parsedSectionsArray.length > 0 && (
                  <select
                    ...
                    className="bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold text-[12px] rounded-lg px-3 py-1.5 outline-none focus:border-indigo-400 shadow-sm cursor-pointer shrink-0"
                  >
                    <option value="">-- No Section --</option>
                    {parsedSectionsArray.map((s) => (
              ...
              <div className="flex items-center gap-3 shrink-0 ml-auto sm:ml-0">
                ...
                <div className="flex items-center bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                  <span className="bg-slate-50 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest px-2.5 py-1.5 border-r border-slate-200">
                    Marks
                  </span>
                  <input
                    type="number"
                    value={q.marks}
                    onChange={(e) =>
                      updateQ(i, "marks", Number(e.target.value))
                    }
                    className="w-14 text-center py-1 text-[14px] font-black text-slate-800 outline-none"
                    style={{
                ...
                <button
                  className="w-8 h-8 rounded-lg bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white border border-rose-100 transition-all flex items-center justify-center shrink-0"
                  onClick={() => rmQ(i)}
            ...
            <div className="p-4 grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-6">
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <label className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest mb-1.5 block">
                    Question Text
                  </label>
                  <textarea
                    value={q.text}
                    onChange={(e) => updateQ(i, "text", e.target.value)}
                    placeholder="Type your question here (MathJax $...$ supported)..."
                    className="w-full p-3 bg-slate-50/50 border border-slate-200 rounded-xl text-[13px] font-medium text-slate-800 outline-none focus:border-blue-400 focus:bg-white transition-all min-h-[80px] resize-y custom-scrollbar"
                  />
                </div>
                ...
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="flex items-center justify-between gap-3 mb-3">
                    <label className="text-[12px] font-extrabold text-slate-700 flex items-center gap-1.5 m-0">
                      <i className="ti ti-vector text-indigo-500 text-base"></i>{" "}
                      Figure / Diagram
                    </label>
                    <select
                  ...
                  {q.figureType === "image" && (
                    <div className="p-3 bg-white border border-dashed border-slate-300 rounded-lg text-center">
                      <label className="cursor-pointer bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-md text-[11px] font-bold inline-flex items-center gap-1.5 transition-colors">
                        <i className="ti ti-upload text-base"></i> Upload
                        Diagram
                        <input
                          type="file"
                          accept="image/*"
                      ...
                      {q.figureData && (
                        <div className="mt-3 flex justify-center">
                          <img
                            src={q.figureData}
                            alt="Preview"
                            className="max-w-full max-h-[120px] rounded border border-slate-200 shadow-sm object-contain"
                          />
                        </div>
                      )}
                    </div>
                  ...
                  {q.figureType === "url" && (
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-400 transition-colors"
                    />
                  )}
                  ...
                  {q.figureType === "svg" && (
                    <div className="flex flex-col gap-2">
                      <textarea
                        value={q.figureData || ""}
                        onChange={(e) =>
                          updateQ(i, "figureData", e.target.value)
                        }
                        placeholder="<svg viewBox='0 0 200 100'> ... </svg>"
                        className="w-full p-2.5 bg-slate-900 text-blue-300 border border-slate-800 rounded-lg text-[10px] font-mono outline-none focus:border-blue-500 transition-colors min-h-[60px] custom-scrollbar"
                      />
                      {q.figureData && (
                        <div
                          className="p-3 bg-white border border-slate-200 rounded-lg flex justify-center shadow-inner overflow-hidden max-w-full"
                          dangerouslySetInnerHTML={{ __html: q.figureData }}
                        ></div>
                      )}
                    </div>
                  )}
                  ...
                  {q.figureType === "smiles" && (
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white border border-indigo-200 rounded-lg text-xs font-mono outline-none focus:border-indigo-400 transition-colors"
                    />
                  )}
                        ...
                        <div className="mt-1 flex justify-center bg-white p-2 rounded-lg border border-slate-200 shadow-inner max-w-full overflow-x-auto hide-scroll">
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.target.src =
                                "https://via.placeholder.com/400x150/f8fafc/ef4444?text=TikZ+Syntax+Error";
                            }}
                          />
                        </div>
                      )}
              ...
              <div className="flex flex-col gap-4">
                ...
                {(q.type === "mcq" || q.type === "msq") && (
                  <div>
                    <label className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest mb-2 block">
                      ...
                      <span
                        className={
                          q.type === "mcq" ? "text-blue-500" : "text-indigo-500"
                        }
                      >
                        {q.type === "mcq"
                          ? "Select correct one"
                    ...
                    <div className="flex flex-col gap-2">
                      {q.options.map((opt, j) => (
                        <div
                          key={j}
                          className={`flex items-center gap-2 p-1.5 pr-3 rounded-lg border transition-all ${q.correct.includes(j) ? "bg-emerald-50 border-emerald-300 shadow-sm" : "bg-white border-slate-200 hover:border-blue-300"}`}
                        >
                          <div className="shrink-0 pl-1.5">
                            {q.type === "mcq" ? (
                              ...
                              <input
                                ...
                                checked={q.correct.includes(j)}
                                onChange={(e) =>
                                  toggleCorrectMSQ(i, j, e.target.checked)
                                }
                          ...
                          <div
                            className={`w-6 h-6 rounded flex items-center justify-center text-[11px] font-black shrink-0 ${q.correct.includes(j) ? "bg-emerald-200 text-emerald-800" : "bg-slate-100 text-slate-500"}`}
                          >
                          ...
                          <input
                            ...
                            className="flex-1 bg-transparent border-none text-[13px] font-medium text-slate-700 outline-none"
                          />
                        </div>
                      ))}
                    </div>
                    <button
                      className="mt-2.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 font-bold text-[11px] rounded-md transition-colors flex items-center gap-1.5 w-fit"
                      onClick={() => addOpt(i)}
                    >
                ...
                {q.type === "integer" && (
                  <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-xl">
                    <label className="text-[11px] font-extrabold text-emerald-700 uppercase tracking-widest mb-1.5 block">
                      Correct Integer Answer
                    </label>
                    <input
                      type="number"
                      value={q.correctInt !== null ? q.correctInt : ""}
                      onChange={(e) =>
                        updateQ(
                          i,
                          "correctInt",
                      ...
                      className="w-28 px-3 py-2 bg-white border border-emerald-200 rounded-lg text-base font-black text-emerald-700 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 transition-all text-center"
                      style={{
                        WebkitAppearance: "none",
                        MozAppearance: "textfield",
                      }}
                ...
                {q.type === "subjective" && (
                  <div className="bg-amber-50/50 border border-amber-100 p-3 rounded-xl">
                    <label className="text-[11px] font-extrabold text-amber-700 uppercase tracking-widest mb-1.5 block">
                      Model Answer (Reference)
                    </label>
                    <textarea
                      value={q.modelAnswer || ""}
                      onChange={(e) =>
                        updateQ(i, "modelAnswer", e.target.value)
                      }
                      placeholder="Write key points for evaluation..."
                      className="w-full p-2.5 bg-white border border-amber-200 rounded-lg text-[13px] font-medium text-amber-900 outline-none focus:border-amber-400 transition-all min-h-[60px] resize-y custom-scrollbar"
                    />
                  </div>
                )}
                ...
                <div className="mt-auto pt-4 border-t border-slate-100">
                  <label className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                    <i className="ti ti-bulb text-amber-500 text-base"></i>{" "}
                    Solution / Logic
                  </label>
                  <textarea
                    value={q.explanation || ""}
                    onChange={(e) => updateQ(i, "explanation", e.target.value)}
                    placeholder="Formula or step-by-step logic..."
                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-[13px] font-medium text-slate-700 outline-none focus:border-blue-400 transition-all min-h-[60px] resize-y custom-scrollbar"
                  />
                </div>
              </div>
            </div>
            ...
            <button
              className="w-full py-2 bg-slate-50/50 border-t border-slate-100 hover:bg-blue-50 text-slate-400 hover:text-blue-600 font-bold text-[11px] uppercase tracking-widest transition-all flex items-center justify-center gap-1.5"
              onClick={() => addQ(i)}
            >
              <i className="ti ti-row-insert-bottom text-sm"></i> Insert
              Question Below
            </button>
          </div>
        ))}
      ...
      <div className="text-center mt-6">
        <button
          className="px-6 py-3 bg-white border border-dashed border-slate-300 text-slate-600 hover:text-blue-600 hover:border-blue-400 font-black text-sm rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] hover:shadow-md transition-all active:scale-95 flex items-center justify-center gap-2 mx-auto"
          onClick={() => addQ()}
        >
        ...
        >
          <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-6 sm:p-8 text-center animate-[popIn_0.3s_ease] border border-slate-100">
            ...
            <p className="text-sm font-semibold text-slate-500 mb-6 leading-relaxed">
              You entered Total Marks as{" "}
              <strong className="text-rose-500">{mismatchModal.entered}</strong>
              , but the questions sum up to{" "}
              <strong className="text-blue-600">{mismatchModal.actual}</strong>.
              <br />
              <br />
              Should we automatically update the total to{" "}
              <strong>{mismatchModal.actual}</strong> and publish?
            </p>
      ...
      {limitExceededModal && (
        ...
        >
          <div
            className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.4)] text-center animate-[popIn_0.3s_cubic-bezier(0.16,1,0.3,1)] border border-slate-200 flex flex-col"
          ...
          >
            <div className="p-8 pb-6 bg-slate-50 flex flex-col items-center border-b border-slate-100">
              ...
              <p className="text-sm font-semibold text-slate-500 mt-2">
                You need at least <strong>1 Test Token</strong> to publish this
                exam. Don't worry, your test is saved as a Draft!
              </p>
            </div>
      ...
      {successModal && (
        ...
        >
          <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-8 text-center animate-[popIn_0.3s_ease]">
            <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center text-4xl mx-auto mb-5 border-2 border-emerald-100 shadow-sm">
            ...
            <p className="text-sm font-semibold text-slate-500 mb-6">
              Your test is ready and locked. Share the code below with your
              students to begin.
            </p>
            ...
            <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl p-5 mb-6 relative">
              ...
              <div className="text-4xl font-black text-blue-600 tracking-[0.2em] font-mono">
                {successModal.code}
              </div>
              <div
                className={`text-[11px] font-extrabold mt-3 flex items-center justify-center gap-1 ${successModal.mode.includes("Local") ? "text-amber-600" : "text-emerald-600"}`}
              >
                <i
                  className={`ti ${successModal.mode.includes("Local") ? "ti-device-floppy" : "ti-cloud-check"} text-sm`}
                ></i>{" "}
                Saved to {successModal.mode}
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-[popIn_0.3s_ease]">
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-[18px] font-black text-slate-800 m-0 leading-none tracking-tight">
                    Paste JSON Data
                  </h3>
                  <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                    Paste your Universal Template JSON here
                  </p>
                </div>
              </div>
              <button
                className="w-8 h-8 rounded-full bg-slate-200 text-slate-500 hover:bg-slate-300 transition-colors"
            ...
            <div className="p-4 sm:p-5 bg-white border-t border-slate-200 flex justify-end gap-3 shrink-0">
              <button
                className="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm rounded-xl transition-colors active:scale-95"
                onClick={() => setPasteModal(false)}
              >
                Cancel
              </button>
              <button
                className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-xl shadow-md shadow-indigo-600/20 transition-all active:scale-95 flex items-center gap-2"
                onClick={() => {
                  if (!pasteText.trim()) {
                    setSysAlert({
                      title: "Empty Input",
                      msg: "Please paste some JSON code first.",
                      type: "warning",
                    });
                    return;
                  }
      ...
      {jsonModal && (
        ...
        >
          <div className="bg-white w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col h-[85vh] animate-[popIn_0.3s_ease]">
            ...
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-rose-100 text-rose-600 rounded-xl flex items-center justify-center text-xl shadow-inner border border-rose-200">
                  <i className="ti ti-code-asterix"></i>
                </div>
                <div>
                  <h3 className="text-[18px] font-black text-slate-800 m-0 leading-none tracking-tight">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-slate-100/50">
              ...
              <div className="lg:w-1/3 bg-white border-b lg:border-b-0 lg:border-r border-slate-200 p-5 overflow-y-auto custom-scrollbar shrink-0">
                <h4 className="text-[12px] font-extrabold text-rose-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <i className="ti ti-bug"></i> Detected Errors (
                  {jsonModal.errors.length})
                </h4>
                <div className="flex flex-col gap-2">
                  {jsonModal.errors.map((err, i) => (
                    <div
                      key={i}
                      className="p-3 bg-rose-50 border border-rose-100 rounded-xl text-[12px] font-semibold text-rose-700 leading-snug shadow-sm"
                    >
                      {err}
                    </div>
                  ))}
                ...
                <div className="mt-5 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                  <h4 className="text-[11px] font-extrabold text-blue-700 uppercase tracking-widest mb-1">
                    Quick Tip
                  </h4>
                  <p className="text-[11px] font-medium text-blue-600/80 leading-relaxed">
                    Find the exact question number in the editor on the right,
                    correct the missing values (like adding [0] to correct
                    arrays), and click Re-Validate.
                  </p>
                </div>
              </div>
              ...
              <div className="flex-1 p-4 sm:p-5 flex flex-col bg-[#0d1117]">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[11px] font-bold font-mono text-slate-400 bg-slate-800 px-3 py-1 rounded-md">
                    test_data.json
                  </span>
                  <span className="text-[11px] font-bold text-emerald-400 flex items-center gap-1">
                    <i className="ti ti-edit"></i> Live Editable
                  </span>
                </div>
                <textarea
            ...
            <div className="p-4 sm:p-5 bg-white border-t border-slate-200 flex justify-end gap-3 shrink-0">
              ...
              <button
                className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm rounded-xl shadow-md shadow-emerald-500/20 transition-all active:scale-95 flex items-center gap-2"
                onClick={() => validateAndImportJSON(jsonModal.text)}
              >
